...
{
	return abs((a1 * pow(x, 2.0) + a2 * pow(z, 2.0) - 1.0));
}
...
void main(){
	...
	if (d >= 0)
	{
		...
		float T = turbulence(fst);	
		float g = getG(gl_FragCoord.x / screenWidth - 0.5, gl_FragCoord.y / screenHeight - 0.5);
		
		float TR = 1.0 - (T - T1 - (T2 - T1) * (1.0 - g)) / D;
		
		color = vec4(vec3(TR), 1.0);
	}
	else
