...
//
////////////////////////////////////////////////////////////
int Image::readDDS( const char *file, Image& i) {

    // open file
	std::fstream f;
	f.open(file, std::ifstream::binary | std::fstream::in);
	if (!f.is_open()){
    ...
    char filecode[4];
	f.rdbuf()->sgetn(filecode, 4);
	//f.read(filecode, 4);
    if (strncmp(filecode, "DDS ", 4) != 0)
    {
		fprintf(stderr, "Bad File Code: %s", filecode);
        return 4;
    }

    // read in DDS header
    DDS_HEADER ddsh;
	f.read(reinterpret_cast<char*>(&ddsh), sizeof(DDS_HEADER));
    ...
    if ( ddsh.dwCaps2 & DDSF_CUBEMAP) {
        ...
        if ( (i._faces != 6) || (i._width != i._height) ) {
			fprintf(stderr, "Not Complete Cube Map");
            return 5;
        }
    }
    else {
    ...
    {
		fprintf(stderr, "Unsupported format 2!\r\n");
        return 7;
    }
    ...
    for (int face = 0; face < ((i._faces) ? i._faces : 1); face++) {
        ...
        for (int level = 0; level < i._levelCount; level++) {
			...
			f.read(reinterpret_cast<char*>(data), sizeof(GLubyte)*size);
